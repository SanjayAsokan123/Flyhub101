  ...
  Future<void> _loginUser() async {
    ...
    try {
      ...
      if (!_looksLikeEmail(input)) {
        ...
        // 1) Try phone lookup
        final phoneDoc = await _firestore.collection('loginIndex').doc('phone$input').get();
        if (phoneDoc.exists &&
            phoneDoc.data() != null &&
            phoneDoc.data()!.containsKey('uid')) {
          final uid = phoneDoc.data()!['uid'] as String;
          emailToUse = await _getEmailForUid(uid);
        } else {
          // 2) Try sellerId/customId lookup
          final sellerIdDoc = await _firestore
              .collection('loginIndex')
              .doc('sellerId_$input')
              .get();
          if (sellerIdDoc.exists &&
              sellerIdDoc.data() != null &&
              sellerIdDoc.data()!.containsKey('uid')) {
            ...
            if (phoneQuery.docs.isNotEmpty) {
              ...
              if (customQuery.docs.isNotEmpty) {
                emailToUse =
                (customQuery.docs.first.data()['email'] ?? "") as String;
              } else {
                throw Exception(
                    "No account found for this email / phone / ID.");
              }
            }
          }
        }
