...
Future<void> main() async {
  ...
  final String graphqlEndpoint = EnvConfig.baseUrl; // e.g. http://192.168.1.178:5001/graphql
...
class _MyAppState extends State<MyApp> {
  ...
  bool _buyerSubAttached = false; // ✅ avoid double subscription
  ...
  Future<void> _initializeApp() async {
    try {
      ...
      if (user != null) {
        if (role == "buyer") {
          final id = await RoleManager.getBuyerId();
          if (id != null) saveBuyerFcmToken(id);
        } else if (role == "seller") {
          final id = await RoleManager.getSellerId();
          if (id != null) saveSellerFcmToken(id);
        }
      }
