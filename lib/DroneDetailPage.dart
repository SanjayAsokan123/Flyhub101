...
class _DroneDetailPageState extends State<DroneDetailPage> {
  ...
  // --------------------------
  // Wishlist Toggle (Provider)
  // --------------------------
  Future<void> _toggleWishlist() async {
    final provider = context.read<CartWishlistProvider>();
    final name = widget.drone['name'] ?? 'Item';
    final productId = widget.drone['productId']?.toString() ??
        widget.drone['id']?.toString() ??
        "";
  ...
  bool _isInWishlist(BuildContext context) {
    ...
    final id = widget.drone['productId']?.toString() ?? widget.drone['id'].toString();
  ...
  Future<void> _addToCart() async {
    final provider = context.read<CartWishlistProvider>();
    final productId = widget.drone['productId']?.toString() ?? widget.drone['id'];
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    image: DecorationImage(
                      image: (drone['image'] != null && drone['image'].toString().isNotEmpty)
                          ? NetworkImage(drone['image'])
                          : const AssetImage('assets/images/MaskGroup34@2x.png')
                      as ImageProvider,
                      fit: BoxFit.cover,
                    ),
                  ),
