...
class _MyCartPageState extends State<MyCartPage>
  ...
  List<Map<String, dynamic>> get cartItems =>
      context.watch<CartWishlistProvider>().cartItems;
  ...
  Future<void> _removeItem(int index) async {
    ...
    await provider.fetchCart(); // refresh UI
  ...
  Future<void> _decreaseQuantity(int index) async {
    ...
    if (qty > 1) {
      await provider.updateCartQty(productId, qty - 1);
      await provider.fetchCart();
  ...
  Widget _buildCartBody() {
    return Column(
      children: [
        if (subtotal > 0 && subtotal < 2000) _buildFreeDeliveryBanner(),
        Expanded(
            ...
            child: RichText(
              text: TextSpan(
                ...
                children: [
                  ...
                  TextSpan(
                    text:
                    "₹${(2000 - subtotal).toStringAsFixed(2)}",
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                      color: accentColor,
                    ),
          ...
          children: [
            Row(
              ...
              children: [
                _cartImage(item),
                const SizedBox(width: 12),
                Expanded(child: _cartItemDetails(item, index)),
              ],
            ),
            const SizedBox(height: 12),
            Divider(color: Colors.grey.shade200, height: 1),
            const SizedBox(height: 12),
            _cartActionButtons(index),
          ],
        ),
      ),
    );
  ...
  Widget _cartImage(Map<String, dynamic> item) {
    return ClipRRect(
      borderRadius: BorderRadius.circular(12),
      child: Image.network(
        item["image"] ?? "",
  ...
  Widget _cartItemDetails(Map<String, dynamic> item, int index) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
        ...
        const SizedBox(height: 12),
        _quantitySelector(item, index),
      ],
    );
  }

  Widget _quantitySelector(Map<String, dynamic> item, int index) {
    return Container(
      decoration: BoxDecoration(
  ...
  Widget _cartActionButtons(int index) {
    return Row(
